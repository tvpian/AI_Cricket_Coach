...
var batMaterial1 = Physijs.createMaterial(new THREE.MeshBasicMaterial({color: 0Xc9b92b}), 0 , 0.99);
...
bat1.rotation.y = 0;
bat1.rotation.z = 0;
bat1.position.set(-0.5, 1.2, -9.1);

bat2.name = 'Batsman2';
bat2.rotation.x = 0;
bat2.rotation.y = 0;
bat2.rotation.z = 0;
//bat2.position.set(-0.5, 1.2, -9.1);
bat2.position.set(100,100, 100);
...
var i=-1;
function moveBat1(xyz1){
  i=i+1
  if(i==7){
	  i=0;
	}
  bat1.position.set(xyz1[i][0], xyz1[i][1], xyz1[i][2]);
  bat1.rotation.x=xyz1[i][3];
  bat1.rotation.y=xyz1[i][4];
  scene.add(bat1);
}

var j=-1;
function moveBat2(xyz2){
  j=j+1
  if(j==7){
	  j=0;
	}
  bat2.position.set(xyz2[j][0], xyz2[j][1], xyz2[j][2]);
  bat2.rotation.x=xyz2[j][3];
  bat2.rotation.y=xyz2[j][4];
  scene.add(bat2);
}

function shot1() {
  moveBat2Coordinates=[[-0.5, 1.2, -9.1,0,-1,0],[-0.55, 1.2, -8.5,0,-1,0],[-0.58, 1.2, -8.3,0,-1,0],[-0.6, 1.2, -8.0,0,-1,0],[-0.65, 1.2, -7.8,-15,-1,0],[-0.68, 1.2, -7.7,-19,-1,0],[-0.7, 1.2, -7.5,-20,-1,0],[-0.72, 1.2, -7.3,-25,-1,0]];
  score=send_bat_coord();
  document.getElementById("ScoreValue").innerHTML = score+"%";
}

function shot2() {
  moveBat2Coordinates=[[-0.7, 1.2, -9.1,0,0,0],[-0.7, 1.2, -8.5,0,0,0],[-0.7, 1.2, -8.3,0,0,0],[-0.7, 1.2, -8.0,0,0,0],[-0.7, 1.2, -7.8,0,0,0],[-0.7, 1.2, -7.7,-10,0,0],[-0.7, 1.2, -7.5,-25,0,0],[-0.7, 1.2, -7.3,-27,0,0]];
  score=send_bat_coord();
  document.getElementById("ScoreValue").innerHTML = score+"%";
}
...
function send_bat_coord()
{
	
	//alert(cids);
	//$.post("/coach",{"bat_coord":"hello"},function(data,status)
	//{
	//	var tmp = data;            
	//	console.log(data.otstr);                     
	//});      
	
//var player_shot=moveBat1Coordinates;
//var reference_shot=moveBat2Coordinates;

var postData = {
  player_shot: moveBat1Coordinates,
  reference_shot: moveBat2Coordinates
}
$.ajax({
    url: "/coach",
    type: "POST",
...
//var testmat = Physijs.createMaterial(new THREE.MeshBasicMaterial({color : white}), 0, 0.99);

//var objLoader = new THREE.OBJLoader();
//function loader(material)
//		{
//				console.log(" Original Material loaded");
//				objLoader.load("Tshirt_obj.obj", function(mesh){
//
...
//function addbat(delta) 
//{
//  var batGeometry = new THREE.BoxGeometry(0.5, 3.5, .2, 64, 64, false);
//  var batMaterial = new THREE.MeshLambertMaterial({color : brown})
//  var bat = new Physijs.CylinderMesh(batGeometry,batMaterial, 0); 
//  bat.name = 'Batsman';
 // bat.rotation.x = 0;
//bat.rotation.y = -1;
//  bat.rotation.z = 0;
//  bat.position.set(-0.5, 1.2+delta, -9.1);
//  scene.add(bat);
...
function animate() {
	...
	moveBat1(moveBat1Coordinates);
	moveBat2(moveBat2Coordinates);
	//Realtime shot type from according to the batsman selected gets stored in the shot
	//console.log("Shot_type: ", shot[0])
	//console.log("Shot_type_value: ", shot[0].value)
